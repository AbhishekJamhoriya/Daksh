    ...
    try:
        ...
        if data:
            ...
            for each in data:
                ...
                #query = "SELECT topic,{} FROM device_readings WHERE topic = '{}' AND time > now() - 10s ORDER BY time DESC LIMIT 1".format(channels, topic)
                query_list.append(query)
            query = "; ".join(query_list)
            #logger.info("Query - {}".format(query))
        ...
        logger.info("WS: Connection Closed.\tjob_id={}".format(job_id))
        scheduler.shutdown()
    except Exception as e:
        scheduler.remove_job('client_{}'.format(job_id))
        logger.error("Exception - {}.\tjob_id={}".format(e, job_id))
        scheduler.shutdown()

async def f(query, websocket, job_id, scheduler, retry=False):
    #global scheduler
    if retry: 
        logger.info('retrying. Time = {}'.format(datetime.now()))
    t1=datetime.now()
    rs_list = client.query(query=query)
    t1_1=datetime.now()
    rs_list = [rs_list] if isinstance(rs_list, ResultSet) else rs_list
    #logger.info('len(rs_list) = {}'.format(len(rs_list)))
    if len(rs_list)<15: 
        logger.error('len(rs_list) < 15, = {}'.format(len(rs_list)))
    response_list = []
    for rs in rs_list:
        device_readings = list(rs.get_points(measurement='device_readings'))
        #if datetime.now().second==0:
        #    logger.info('device_readings = {}'.format(device_readings))              
        if len(device_readings)==0: 
            logger.info('device_readings = {}'.format(device_readings))              
            if not retry: 
                await f(query, websocket, job_id, scheduler, retry=True)
                return

        local_tz_device_readings = convert_device_readings_to_local_timezone(device_readings)
        response_list.extend(local_tz_device_readings)

    #logger.info(f'len(response_list) before = {len(response_list)}')
    ...
    try:
        await websocket.send_json(response_list)
        if len(response_list)<15: logger.info(f'len(response_list) = {len(response_list)}')
        t3 = datetime.now()
        if datetime.now().second==0:
            logger.info('Query duration : {} and processing duration : {}, start={}, querier={}, processed={}, pushed={}. Total duration : {}'.format(t1_1-t1, t1_1-t1, t2-t1, t1, t2, t3, t3-t1))
    except (ConnectionClosedOK, WebSocketDisconnect) as e:
        scheduler.remove_job('client_{}'.format(job_id))
        #logger.info(websocket.client_state)
        logger.info("JOB : Connection Closed.\tjob_id={}".format(job_id))
        scheduler.shutdown()
    except Exception as e:
        scheduler.remove_job('client_{}'.format(job_id))
        #logger.info(websocket.client_state)
        logger.error("JOB : Exception : {}.\tjob_id={}".format(e, job_id))
        scheduler.shutdown()
    #websocket.send_json(response_list)

def convert_device_readings_to_local_timezone(device_readings):
    #if datetime.now().second==0:
    #    logger.info('len(device_readings) = {}'.format(len(device_readings)))
